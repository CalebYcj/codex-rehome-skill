---
name: codex-rehome
description: "Advanced workflow for migrating, backing up, restoring, or reproducing a Codex Desktop workspace between Mac and Windows computers; use for automation, verification, no-GUI environments, and recovery when ReHome Desktop is not the right entry point."
---

# Codex Rehome

> Most users should use [ReHome Desktop](https://github.com/CalebYcj/codex-rehome). This Skill remains the advanced, agent-operated route for automation, audits, no-GUI environments, and troubleshooting.
...
6. Verify continuity.
   ...
   - On Windows, fully close Codex Desktop, ChatGPT, `extension-host`, plugin app-server processes, and their child processes before restore. After restoring, open Codex once, wait for it to finish loading, fully close it again, and rerun the verifier. Require `sqlite_quick_check_ready=true` and `structural_source_paths_ready=true`; this catches a surviving process writing old paths back.
   - Rewrite only structural path fields such as `cwd`, `project_path`, `workspace_root`, and `rollout_path`. Preserve titles, prompts, messages, previews, and other historical text even when they mention an old username or path. Historical image and attachment references outside known structural fields are not guaranteed to remain directly openable.
...
```text
I just reinstalled this computer. This folder contains a Codex migration package created with Codex ReHome Skill before reinstall. Use the latest instructions from https://github.com/CalebYcj/codex-rehome-skill, unzip the package, run the restore script for this OS with project restore enabled, restore projects into the default Codex-Restored-Projects folder, register/open restored projects in Codex Desktop, then run the verifier and report which sessions, skills, plugins, and projects restored successfully. Do not restore auth tokens, browser cookies, .env files, private keys, node_modules, .git, or virtual environments.
```
